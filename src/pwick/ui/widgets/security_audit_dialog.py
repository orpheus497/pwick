...
from PySide6.QtWidgets import (
    ...
    QPushButton, QLabel, QListWidget, QListWidgetItem, QGroupBox, QMessageBox
)
...
class SecurityAuditDialog(QDialog):
    ...
    def _run_audit(self):
        ...
        self.duplicate_groups = [
            entries for entries in password_map.values()
            if len(entries) > 1
        ]
    ...
    def _setup_ui(self):
        ...
        summary_label.setStyleSheet("padding: 10px; background-color: #2d2d2d; border-radius: 4px;")
        layout.addWidget(summary_label)

        # Create tab widget
        self.tabs = QTabWidget()
        self.tabs.addTab(self._create_duplicates_tab(), f"Duplicate Passwords ({len(self.duplicate_groups)})")
        self.tabs.addTab(self._create_weak_passwords_tab(), f"Weak Passwords ({len(self.weak_entries)})")
    ...
    def _get_summary_text(self) -> str:
        ...
        if self.duplicate_groups:
            ...
            parts.append(f"• {dup_count} entries using {len(self.duplicate_groups)} duplicate password(s)")

        if self.weak_entries:
            parts.append(f"• {len(self.weak_entries)} weak password(s) that should be upgraded")
    ...
    def _create_duplicates_tab(self) -> QWidget:
        ...
        if not self.duplicate_groups:
            no_issues = QLabel("✓ No duplicate passwords found!\n\n"
    ...
    def _create_weak_passwords_tab(self) -> QWidget:
        ...
        if not self.weak_entries:
            no_issues = QLabel("✓ No weak passwords found!\n\n"
            ...
            for weak in self.weak_entries:
                ...
                # Create display text
                strength_text = ["Very Weak", "Weak", "Fair"][score] if score < 3 else "Fair"
                item_text = f"{entry['title']}"
                if entry.get('username'):
